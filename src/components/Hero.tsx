...
export default function Hero({ data, onReserveClick, isAdminMode }: HeroProps) {
  ...
  return (
    <section id="home" className="relative h-screen w-full overflow-hidden bg-white">
      ...
      {/* Controls: Sound & Custom Video Select */}
      <div className="absolute bottom-12 left-6 md:left-12 z-30 flex items-center space-x-3">
        <button
          onClick={toggleMute}
          className="flex items-center space-x-3 bg-white/70 backdrop-blur-md border border-gold/30 hover:border-gold hover:bg-gold/10 text-gold-dark hover:text-luxury-black px-4 py-2.5 rounded-full text-[10px] uppercase tracking-[0.2em] transition-all duration-300 cursor-pointer"
        >
          {isMuted ? (
            <>
