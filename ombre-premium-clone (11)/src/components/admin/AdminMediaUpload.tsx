...
export const AdminMediaUpload: React.FC<AdminMediaUploadProps> = ({
  ...
  // A value is only ever safe to display in the browser if it's a real
  // http(s) URL or a base64 data URI. Anything else (most commonly a local
  // file path like "C:\Users\...\photo.jpg" pasted by mistake from Windows
  // Explorer, or a bare filename) can never load for site visitors and must
  // be rejected before it gets saved.
  const isValidMediaUrl = (val: string) => {
    if (!val) return true; // empty is allowed (clears the field)
    return /^(https?:\/\/|data:)/i.test(val.trim());
  };
  ...
  return (
    <div id={`${id}-wrapper`} className={`space-y-2 ${className}`}>
      ...
      <div className="flex flex-col gap-3">
        ...
        <div className="flex gap-2">
          <input
            ...
            onChange={(e) => {
              ...
              if (isValidMediaUrl(newVal)) {
                setUrlWarning("");
                onChange(newVal);
              } else {
                // Do not save invalid values (e.g. local file paths like
                // "C:\Users\...\photo.jpg") — this is what silently broke
                // images across every device before.
                setUrlWarning(
                  "That doesn't look like a web link (starting with http:// or https://). Local file paths can't be shown on the website — please use the upload box below instead."
                );
              }
            }}
        ...
        {/* Invalid manual URL warning (e.g. pasted a local file path) */}
        {urlWarning && (
          <div className="flex items-center gap-1.5 text-xs text-red-400 bg-red-950/20 border border-red-900/30 rounded px-3 py-2">
            <AlertCircle className="w-3.5 h-3.5 flex-shrink-0" />
